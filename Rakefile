...
task :test_database_setup do
  ...
  connection.exec("TRUNCATE comments, links, tags, link_tags, users;")
end
...
task :setup do
  ...
  ['bookmark_manager', 'bookmark_manager_test'].each do |database|
    ...
    connection.exec("CREATE TABLE users(id SERIAL PRIMARY KEY, email VARCHAR(60), password VARCHAR(240));")
  end
end
